...
function BrandLogo() {
  return (
    <div className="flex items-center gap-2.5">
      ...
      >
        <defs>
          <linearGradient id="inventoryLogoGold" x1="0" x2="1">
...
export default function InventoryPage() {
  ...
  useEffect(() => {
    ...
    async function loadInventory() {
      ...
      if (error) {
        console.error(error);
        setLoadError("We could not load the inventory. Please try again.");
        setVehicles([]);
      } else {
  ...
  const brands = useMemo(
    () => [...new Set(vehicles.map((vehicle) => vehicle.brand).filter(Boolean))].sort(),
    [vehicles]
  );

  const locations = useMemo(
    () =>
      [...new Set(vehicles.map((vehicle) => vehicle.location).filter(Boolean))].sort() as string[],
    [vehicles]
  );
  ...
  }, [vehicles, search, brand, location, status]);
    ...
    <main className="min-h-screen bg-[#F5EFE6] text-[#211A13]">
      <header className="border-b border-[#B88A3B]/20 bg-[#F8F4ED]/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4 px-5 py-4 md:px-8 lg:px-10">
          <Link href="/" aria-label="MMADUABUCHI MOTORS home">
            <BrandLogo />
          </Link>
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-16 md:px-8 md:py-20 lg:px-10">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1.2fr_.8fr] lg:items-end">
            ...
            <p className="max-w-xl text-sm leading-7 text-[#746653] lg:justify-self-end">
              Explore vehicles currently listed by MMADUABUCHI MOTORS across
              Cotonou, Lagos and Onitsha. Availability is managed directly from
              the dealership admin dashboard.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1500px] px-5 py-10 md:px-8 lg:px-10">
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-[1.5fr_.7fr_.7fr_.7fr]">
            <label className="relative block">
              ...
              <input
                ...
                className="h-13 w-full border border-[#CBB895]/45 bg-white pl-11 pr-4 text-sm outline-none transition focus:border-[#B88932]"
              />
            </label>
            ...
            <select
              ...
              className="h-13 border border-[#CBB895]/45 bg-white px-4 text-sm outline-none focus:border-[#B88932]"
            >
        ...
        <div className="mb-7 mt-10 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="font-display mt-2 text-3xl sm:text-4xl">
              {loading ? "Loading inventory..." : `${filteredVehicles.length} vehicle${filteredVehicles.length === 1 ? "" : "s"}`}
            </h2>
          </div>

          {(search !== "" || brand !== "all" || location !== "all" || status !== "all") && (
            <button
              type="button"
              onClick={() => {
                setSearch("");
        ...
        {loading && (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="overflow-hidden border border-[#B88A3B]/15 bg-[#FAF8F3]">
        ...
        {!loading && !loadError && filteredVehicles.length === 0 && (
          <div className="border border-[#B88A3B]/20 bg-[#FAF8F3] px-6 py-20 text-center">
            ...
            <h3 className="font-display mt-5 text-3xl">No vehicles match your search.</h3>
            <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-[#796B59]">
              Try another brand, location or status, or contact MMADUABUCHI
              MOTORS directly for current availability.
            </p>
          </div>
        )}
        ...
        {!loading && !loadError && filteredVehicles.length > 0 && (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {filteredVehicles.map((vehicle, index) => {
              const whatsappMessage = `Hello MMADUABUCHI MOTORS, I'm interested in the ${
                vehicle.year ? `${vehicle.year} ` : ""
              }${vehicle.brand} ${vehicle.model}. Please tell me more about availability.`;
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <Link href={`/cars/${vehicle.id}`} className="mt-2 block">
      ...
      <section className="bg-[#211810] px-5 py-16 text-center text-white md:px-8">
        ...
        <p className="mx-auto mt-5 max-w-lg text-sm leading-7 text-[#BBAA93]">
          Speak directly with MMADUABUCHI MOTORS about specifications,
          inspection, sourcing and availability.
        </p>
